...
using WeatherService.Testing.XUnit.ServiceRegistrations.Extensions;
using WeatherService.Testing.XUnit.Unit.Core.Specifications;
using Xunit;

namespace WeatherService.Testing.XUnit.ServiceRegistrations.Specifications;

public abstract class ServiceCollectionTestSpecification<TAssemblyMarker>: TestSpecification
{
    ...
    public static TheoryData<Type> TypesToTest()
    {
        return new TheoryData<Type>(typeof(TAssemblyMarker).Assembly
            .GetTypes()
    ...
    [Theory, MemberData(nameof(TypesToTest))]
